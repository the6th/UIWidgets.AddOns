...
namespace UIWidgets.AddOns
{
    ...
    {
        private static List<string> _kBlockTags = new List<string>()
        {
            "p",
            "h1",
            "h2",
        ...
        private static bool _isBlockTag(string tag)
        {
            return _kBlockTags.Contains(tag);
        }
        private static bool _isListTag(string tag)
        {
        ...
        public class Builder : markdown.NodeVisitor
        {
            /// A delegate that controls how link and `pre` elements behave.
            private IMarkdownBuilderDelegate myDelegate;

            /// Defines which [TextStyle] objects to use for each type of element.
            private MarkdownStyleSheet styleSheet;

            /// The base directory holding images referenced by Img tags with local file paths.
            private string imageDirectory;

            private List<string> _listIndents = new List<string>();
            private List<_BlockElement> _blocks = new List<_BlockElement>();
            private List<_InlineElement> _inlines = new List<_InlineElement>();
            private List<GestureRecognizer> _linkHandlers = new List<GestureRecognizer>();

            /// Creates an object that builds a [Widget] tree from parsed Markdown.
            public Builder(IMarkdownBuilderDelegate myDelegate, MarkdownStyleSheet styleSheet,
                string imgDir)
            {
            ...
            private Widget _buildBullet(string listTag)
            {
                if (listTag == "ul")
                    return new Unity.UIWidgets.widgets.Text("•", style: styleSheet.styles("li"), textAlign: TextAlign.center);

                int index = _blocks.last().nextListIndex;
                return new Padding(null, EdgeInsets.only(5.0f),
                    new Unity.UIWidgets.widgets.Text((index + 1) + ".", style: styleSheet.styles("li"), textAlign: TextAlign.right));
            }

            private void _addParentInlineIfNeeded(string tag)
            {
            ...
            private void _addBlockChild(Widget child)
            {
            ...
            private void _addAnonymousBLockIfNeeded(TextStyle style)
            {
                if (_inlines.isEmpty())
                {
                    return;
            ...
            private List<Widget> _mergeInlineChildren(_InlineElement inline)
            {
